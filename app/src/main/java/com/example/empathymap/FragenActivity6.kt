...
class FragenActivity6 : AppCompatActivity(), RobotLifecycleCallbacks {
    ...
    override fun onRobotFocusGained(qiContext: QiContext?) {
        val btnskip: Button = findViewById(R.id.button_skip6) //skip questions
        val btnpause: Button = findViewById(R.id.button_pause6) //pause timer
        ...
        val say_4 = SayBuilder.with(qiContext)
            .withText("Hier geht es darum. Was ich tun muss um mein Lernziel zu erreichen. ")
            .build()
        val say_5 = SayBuilder.with(qiContext)
            .withText("Die Antwort könnte lauten.")
            .build()
        val say_6 = SayBuilder.with(qiContext)
            .withText("Ich probe und bitte meine Freunde um Feedback.")
            .build()
        ...
        val ballet: Future<Void> = first1.thenCompose {
            ...
            TimeUnit.SECONDS.sleep(1L)
            say_9.async().run() }.thenCompose<Void?> {
            TimeUnit.SECONDS.sleep(1L)
            ...
            timer.scheduleAtFixedRate(500L, 1000L) {
                if (!isPaused) {
                    if (seconds > timeSpanInSec) {
                        isPaused = true
                        timer.cancel()
                        this@FragenActivity6.runOnUiThread(java.lang.Runnable {
